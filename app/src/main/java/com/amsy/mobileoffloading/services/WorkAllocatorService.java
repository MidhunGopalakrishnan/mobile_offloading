...
public class WorkAllocatorService {
    ...
    public WorkAllocatorService(Context context, ArrayList<Worker> workers, int[][] matrix1, int[][] matrix2, ComputationListener uiCallback) {
        ...
        this.matrix2Transpose = MatrixManager.getTranspose(matrix2);
...
*/
    ...
    public void allocateWork() {
        if (workerQueue.size() > 0 && workQueue.size() > 0 && partitionResults.size() != totalpartitions) {
            ...
            payload.setTag(GobalConstants.PayloadTags.WORK_DATA);
            payload.setData(workData);
            try {
                //TODO find out communication here : Done
                Payload payload1 = PayloadInterface.toPayload(payload);
                WorkerConnectorService.sendToDevice(context, worker.getEndpointId(), payload1);
    ...
    public void updateWorkStatus(Worker worker, WorkDataStatus workDataStatus) {
        if (partitionResults.size() == totalpartitions) {
            return; //finished all work :)
        }
        if (worker == null || workDataStatus == null) {
            return; // initialization is not done yet
        }

        if (workDataStatus.getStatusInfo().equals(GobalConstants.WorkStatus.WORKING)) {
            partitionResults.put(workDataStatus.getPartitionIndexInfo(), workDataStatus.getResultInfo());
        }

        if (workDataStatus.getStatusInfo().equals(GobalConstants.WorkStatus.FAILED) || workDataStatus.getStatusInfo().equals(GobalConstants.WorkStatus.DISCONNECTED)) {
            addWorkToQueue(workDataStatus.getPartitionIndexInfo());
        }

        if (!workDataStatus.getStatusInfo().equals(GobalConstants.WorkStatus.DISCONNECTED)) {
    ...
    private void sendByeToWorkers() {
        ...
        for (Worker worker : workers) {
            if (!worker.getWorkStatus().getStatusInfo().equals(GobalConstants.WorkStatus.DISCONNECTED)) {
                ClientPayload payload = new ClientPayload();
                payload.setTag(GobalConstants.PayloadTags.FAREWELL);

                //TODO: Send data to device on connection. Done
                WorkerConnectorService.sendToDevice(context, worker.getEndpointId(), payload);
                WorkerConnectionManagerService.getInstance(context).rejectConnection(worker.getEndpointId());
            }
        }
